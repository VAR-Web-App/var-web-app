...
export interface AssemblyInstance {
  ...
  /**
   * Where this instance came from. "plan" = generated by the plan
   * extractor / from-plan converter. "manual" = the user added it via
   * the Add Assembly modal or hand-edited a plan instance. Drives the
   * smart Replace flow — when a builder replaces the AI plan output,
   * we wipe instances with source="plan" but keep "manual" ones so
   * their custom additions survive. Absent = legacy/unknown; treat
   * as "manual" to err on preservation.
   */
  source?: "plan" | "manual";
